...
try {
    ...
    FROM goods1 g
    JOIN orders1 o ON g.id = o.goods_id
...
<div class="container2">
    ...
    <div class="goodsblock">
        <div class="goods-name">Товар<br> &laquo;<?= htmlspecialchars($good['title']) ?>&raquo;</div><div><br></div>
...
<?php if (!empty($userOrders)): ?>
        <?php foreach ($userOrders as $order): ?>
                <div class="block2">
                <div class="goods-name">Товар &laquo;<?= htmlspecialchars($order['title']) ?>&raquo;</div><div></div><br>
                <div>Цена: <?= htmlspecialchars($order['price']) ?> рублей / месяц</div>
                <!-- В коде ниже переменных description и price у заказа нет, если нужно описание, надо добавить в SQL -->
                </div>
        <?php endforeach; ?>
<?php else: ?>
...
function filterBlocks() {
    ...
    const blocks = document.querySelectorAll('.subscription-block .goodsblock');
